...
class Tally(dbModule):
	...
	def genPairResults_Schulze(self):
		...
		for comb in combinations(self.candidates, 2):
			...
			# formattedrevcands = self.formatCands([x for x in revcomb])
			print "formattedcands: ", formattedcands
			# print "formattedrevcands: ", formattedrevcands
			genballot_cmd = "SELECT DISTINCT ballot_id FROM votes WHERE (cand_id in %s and race_id = \'%i\')" % (formattedcands, self.race_id)
			for ballot_id in self.genBallotID(genballot_cmd):
				...
				cmd2 = "SELECT score FROM votes WHERE cand_id = \'%i\' and ballot_id = \'%i\'" % (
					comb[1], ballot_id)
				print "raw query: ", self.queryDB(cmd1), self.queryDB(cmd2)
				...
				if score1 > score2:
					...
					results[revcomb] += 1
				elif score1 == score2: # Is this legit?  What am I supposed to do with ties?
					results[comb] += 1
					results[revcomb] += 1
